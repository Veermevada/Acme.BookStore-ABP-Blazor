...
{
    public partial class CustomFeatureManagementModal
    {
        ...
        [Inject] protected ICustomIdentityUserRepository? IdentityUserAppService { get; set; }

        [Inject] protected IRepository<IdentityUser, Guid>? IdentityUserRepository { get; set; }

        [Inject] protected ITenantRepository? TenantRepository { get; set; }
        ...
        protected virtual async Task SaveAsync()
        {
            try
            {
                var tenantUser = await IdentityUserAppService!.GetListByTenantId(Guid.Parse(ProviderKey!));
